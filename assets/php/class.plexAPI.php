...
class plexAPI
{
	...
	*/	
	...
	public function getItemsContent($itemsKey, $contentType, $content, $item)
	{
	...
	function getFormatedItemsContent($itemsKey, $contentType, $content, $format, $item)
	{
		...
		switch ($format)
		{
			case 'img': 
			{
				...
				$content = '<img alt="" class="img-rounded img-responsive" src="'.PLPP_BASE_PATH.'?item='.$content.'&thumb='.$thumb.'&viewmode=img&type='.$this->plexItems[$item]['items'][$itemsKey]['type'].'&filename='.$this->plexItems[$item]['items'][$itemsKey]['title'].'">';
				break;
			}
			case 'date':
			{
